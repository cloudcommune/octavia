...
LB3 = o_data_models.LoadBalancer(vip=VIP3, amphorae=AMPS_DATA2)
...
class TestNetworkTasks(base.TestCase):
    ...
    def test_del_one_ecmp_routes(self, mock_get_net_driver):
        ...
        net.execute(self.load_balancer_mock, AMPS_DATA3)
        mock_driver.del_ecmp_routes.assert_called_once_with('router_id',
                                                            'ip_address/32',
                                                            ['vrrp_ip'])

        mock_driver.reset_mock()
        net.revert(result, self.load_balancer_mock, AMPS_DATA3)
        mock_driver.add_ecmp_routes.assert_called_once_with('router_id',
                                                            'ip_address/32',
                                                            ['vrrp_ip'])
