...
class TestAmphoraDriverTasks(base.TestCase):
    ...
    def test_amphora_config_update(self,
        ...
        mock_driver.update_amphora_agent_config.side_effect = [
            None, None, None, driver_except.AmpDriverNotImplementedError,
        ...
        mock_build_config.assert_called_once_with(
            _db_amphora_mock.id, constants.TOPOLOGY_ACTIVE_STANDBY, None, None)
        ...
        mock_build_config.assert_called_once_with(
            _db_amphora_mock.id, constants.TOPOLOGY_SINGLE, None, None)
        mock_driver.update_amphora_agent_config.assert_called_once_with(
            _db_amphora_mock, FAKE_CONFIG_FILE)
        # snat_ip dhcp_ip
        mock_driver.reset_mock()
        mock_build_config.reset_mock()
        flavor2 = {constants.LOADBALANCER_TOPOLOGY:
                   constants.TOPOLOGY_MULTI_ACTIVE}
        mock_amphora_repo_get.return_value = _db_amphora_mock
        amp_config_update_obj.execute(_amphora_mock,
                                      flavor2, 'dhcp_ip', 'snat_ip')
        mock_build_config.assert_called_once_with(
            _db_amphora_mock.id, constants.TOPOLOGY_MULTI_ACTIVE,
            'dhcp_ip', 'snat_ip')
        mock_driver.update_amphora_agent_config.assert_called_once_with(
            _db_amphora_mock, FAKE_CONFIG_FILE)
    ...
    @mock.patch('octavia.db.repositories.LoadBalancerRepository.update')
    @mock.patch('octavia.db.repositories.LoadBalancerRepository.get')
    def test_amphora_post_loop_vip_plug(self, mock_lb_get,
                                        mock_loadbalancer_repo_update,
                                        mock_driver,
                                        mock_generate_uuid,
                                        mock_log,
        ...
        mock_amphora_repo_get.return_value = _db_amphora_mock
        mock_lb_get.return_value = _db_load_balancer_mock
        amphora_post_vip_plug = amphora_driver_tasks.AmphoraPostLoopVipPlug()
        amphora_post_vip_plug.execute(_amphora_mock,
                                      _LB_mock,
                                      amphorae_net_config_mock)
        vip_subnet = network_data_models.Subnet(
            **amphorae_net_config_mock[AMP_ID]['vip_subnet'])
        ...
        mock_driver.post_loop_vip_plug.assert_called_once_with(
            _db_amphora_mock, _db_load_balancer_mock, amphorae_net_config_mock,
            vip_subnet=vip_subnet, vrrp_port=vrrp_port)

        # Test revert
        amp = amphora_post_vip_plug.revert(None, _amphora_mock, _LB_mock)
        repo.AmphoraRepository.update.assert_called_once_with(
            _session_mock,
            id=AMP_ID,
            status=constants.ERROR)
