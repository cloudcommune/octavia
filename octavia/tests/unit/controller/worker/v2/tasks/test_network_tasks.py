...
VIP3 = o_data_models.Vip(port_id=t_constants.MOCK_PORT_ID2,
                         subnet_id="subnet_id",
...
AMPS_DATA2 = [o_data_models.Amphora(id='amphora_id',
                                    vrrp_port_id="vrrp_port_id",
                                    vrrp_ip="vrrp_ip"), ]
AMPS_DATA3 = o_data_models.Amphora(id=t_constants.MOCK_AMP_ID1,
                                   vrrp_port_id="vrrp_port_id",
                                   vrrp_ip="vrrp_ip")
AMPS_DATA4 = o_data_models.Amphora(id='amphora_id',
                                   vrrp_port_id="vrrp_port_id",
                                   vrrp_ip="vrrp_ip").to_dict()
LB3 = o_data_models.LoadBalancer(vip=VIP3,
                                 amphorae=AMPS_DATA2,
...
class TestNetworkTasks(base.TestCase):
    def setUp(self):
        ...
        self.load_balancer_mock = {
            ...
            constants.VIP_NETWORK_ID: VIP.network_id,
            constants.VIP_QOS_POLICY_ID: t_constants.MOCK_QOS_POLICY_ID1
        }
        self.load_balancer_mock2 = {
            constants.LOADBALANCER_ID: 'loadbalance_id',
            constants.VIP_SUBNET_ID: 'vip_address',
            constants.VIP_PORT_ID: 'port_id',
            constants.VIP_ADDRESS: 'vip_address',
    ...
    @mock.patch('octavia.db.api.get_session', return_value='TEST')
    @mock.patch('octavia.db.repositories.LoadBalancerRepository.get')
    def test_add_ecmp_routes(self, mock_lb_get, mock_get_session,
                             mock_get_net_driver):
        mock_driver = mock.MagicMock()
        result = mock.MagicMock()
        mock_get_net_driver.return_value = mock_driver
        mock_lb_get.return_value = LB3
        mock_driver.get_router_id.return_value = "router_id"
        net = network_tasks.AddEcmpRoutes()
        net.execute(self.load_balancer_mock)
        mock_driver.add_ecmp_routes.assert_called_once_with('router_id',
                                                            'ip_address/32',
                                                            ['vrrp_ip'])

        mock_driver.reset_mock()
        net.revert(result, self.load_balancer_mock)
        mock_driver.del_ecmp_routes.assert_called_once_with('router_id',
                                                            'ip_address/32',
                                                            ['vrrp_ip'])

    @mock.patch('octavia.db.api.get_session', return_value='TEST')
    @mock.patch('octavia.db.repositories.LoadBalancerRepository.get')
    def test_del_ecmp_routes(self, mock_lb_get, mock_get_session,
                             mock_get_net_driver):
        mock_driver = mock.MagicMock()
        result = mock.MagicMock()
        mock_get_net_driver.return_value = mock_driver
        mock_lb_get.return_value = LB3
        mock_driver.get_router_id.return_value = "router_id"
        net = network_tasks.DelEcmpRoutes()
        net.execute(self.load_balancer_mock2)
        mock_driver.del_ecmp_routes.assert_called_once_with('router_id',
                                                            'ip_address/32',
                                                            ['vrrp_ip'])
    ...
    def test_del_one_ecmp_routes(self, mock_get_net_driver):
        mock_driver = mock.MagicMock()
        result = mock.MagicMock()
        mock_get_net_driver.return_value = mock_driver
        self.vip_mock = mock.MagicMock()
        ...
        self.vip_mock.ip_address = 'ip_address'
        mock_driver.get_router_id.return_value = "router_id"
        net = network_tasks.DelOneEcmpRoute()
        net.execute(self.load_balancer_mock2, AMPS_DATA3.to_dict())
        mock_driver.del_ecmp_routes.assert_called_once_with('router_id',
                                                            'vip_address/32',
                                                            ['vrrp_ip'])

        mock_driver.reset_mock()
        net.revert(result, self.load_balancer_mock2, AMPS_DATA3.to_dict())
        mock_driver.add_ecmp_routes.assert_called_once_with('router_id',
                                                            'vip_address/32',
                                                            ['vrrp_ip'])

    def test_add_one_ecmp_routes(self, mock_get_net_driver):
        mock_driver = mock.MagicMock()
        mock_get_net_driver.return_value = mock_driver
        mock_driver.get_router_id.return_value = "router_id"
        net = network_tasks.AddOneEcmpRoute()
        net.execute(self.load_balancer_mock2, AMPS_DATA4)
        mock_driver.add_ecmp_routes.assert_called_once_with('router_id',
                                                            'vip_address/32',
                                                            ['vrrp_ip'])
        mock_driver.reset_mock()
        net.revert(self.load_balancer_mock2, AMPS_DATA4)
        mock_driver.del_ecmp_routes.assert_called_once_with('router_id',
                                                            'vip_address/32',
                                                            ['vrrp_ip'])
    ...
    @mock.patch('octavia.db.api.get_session', return_value='TEST')
    @mock.patch('octavia.db.repositories.LoadBalancerRepository.get')
    def test_get_multi_health_check_port_id(self, mock_lb_get,
                                            mock_get_session,
                                            mock_get_net_driver):
        mock_driver = mock.MagicMock()
        mock_get_net_driver.return_value = mock_driver
        mock_lb_get.return_value = LB3
        net = network_tasks.GetMultiHealthCheckPortId()
        net.execute('loadbalance_id')
        mock_driver.get_port_ip.assert_called_with('network_id', 'subnet_id',
