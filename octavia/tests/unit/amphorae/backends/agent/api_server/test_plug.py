...
FAKE_INTERFACE = 'eth33'
    ...
    def test_plug_vip_with_cmd(self, mock_makedirs, mock_copytree,
                               mock_check_output, mock_netns,
                               mock_IPRoute, mock_webob,
                               mock_nsopen, mock_ns):
        interface_ip = '192.168.11.1'
        mock_ns.listnetns.return_value = ['amphora-haproxy']
        m = mock.mock_open()
        with mock.patch('os.open'), mock.patch.object(os, 'fdopen', m):
            self.test_plug.plug_vip_with_cmd(
                vip=FAKE_IP_IPV4,
        ...
        mock_webob.Response.assert_any_call(json=dict(
            message="OK",
            detail="{vip} has been added on network card lo successfully"
                    .format(vip=FAKE_IP_IPV4)), status=200)
        cmd_list = ['ip a add {ip} dev {card}'.format(
            ip=FAKE_IP_IPV4, card='lo').split()]
        pattern_list = ['sysctl -w net.ipv4.conf.{0}.arp_ignore=1',
                        'sysctl -w net.ipv4.conf.{0}.arp_announce=2']
        for dev in ['all', 'lo', 'eth1']:
            for pattern in pattern_list:
                cmd_list.append(pattern.format(dev).split())
        calls = [mock.call('amphora-haproxy', cmd, stdout=subprocess.PIPE)
                 for cmd in cmd_list]
        mock_nsopen.assert_has_calls(calls, any_order=True)

        # new test
        mock_ns.listnetns.return_value = []
        mock_webob.reset_mock()
        mock_nsopen.reset_mock()
