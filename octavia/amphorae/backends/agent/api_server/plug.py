...
class Plug(object):
    ...
    def plug_vip_to_lo(self, vip):
        self._osutils.write_vip_lo_file(
            interface="lo",
            ip_address="127.0.0.1",
            vip_address=vip,
            prefixlen=8)

    def _plug_vip_common(self, vip, ip, prefixlen, gateway, mac_address,
                         mtu=None, vrrp_ip=None, host_routes=None):
    ...
    def plug_vip(self, vip, subnet_cidr, gateway,
        ...
        self._plug_vip_common(vip, ip, prefixlen, gateway,
                              mac_address, mtu, vrrp_ip, host_routes)
        ...
        if init_system == consts.INIT_SYSTEMD:
            ...
            util.run_systemctl_command(consts.ENABLE, consts.AMP_NETNS_SVC_PREFIX)
    ...
    def plug_vip_with_cmd(
            self, vip, mac_address, subnet_cidr, gateway, vrrp_ip=None,
            mtu=None, host_routes=None):
        LOG.info(
            'param list: vip %s, mac_address %s, subnet_cidr %s,'
            ' gateway %s, vrrp_ip %s, mtu %s, host_routes: %s',
            vip, mac_address, subnet_cidr, gateway, vrrp_ip, mtu, host_routes)
        try:
            ip = ipaddress.ip_address(vip)
            network = ipaddress.ip_network(subnet_cidr)
            prefixlen = network.prefixlen
        ...
        self._plug_vip_common(vip, ip, prefixlen, gateway, mac_address, mtu,
                              vrrp_ip=vrrp_ip, host_routes=host_routes)

        netcard = consts.LOOPBACK_INTERFACE
        cmd_list = []
        self.plug_vip_to_lo(vip)
        pattern_list = ['{0} -w net.ipv4.conf.{1}.arp_ignore=1',
                        '{0} -w net.ipv4.conf.{1}.arp_announce=2']
        primary_interface = consts.NETNS_PRIMARY_INTERFACE
        extra_entry = []
        for dev in ['all', consts.LOOPBACK_INTERFACE,
                    primary_interface]:
            for pattern in pattern_list:
                cmd_list.append(pattern.format(consts.SYSCTL_CMD, dev).split())
        for cmd in cmd_list:
            ns_exec = pyroute2.NSPopen(consts.AMPHORA_NAMESPACE, cmd,
                                       stdout=subprocess.PIPE)
            extra_entry.append(" ".join(cmd))
            ns_exec.wait()
            ns_exec.release()

        self._osutils.bring_interfaces_up(ip, primary_interface)
        self._osutils.bring_interfaces_up(ip, netcard)
        init_system = util.get_os_init_system()
        if init_system == consts.INIT_SYSTEMD:
            util.install_netns_systemd_service(extra_entry=extra_entry)
            util.run_systemctl_command(consts.ENABLE, consts.AMP_NETNS_SVC_PREFIX)
        return webob.Response(json=dict(
            message="OK",
            detail="{vip} has been added on network"
                   " card {card} successfully".format(
                    vip=ip.exploded, card=netcard)), status=200)

    def unplug_vip(self, vip):
        try:
            ip = ipaddress.ip_address(vip)
        except ValueError:
            return webob.Response(json=dict(message="Invalid VIP"),
        ...
        self.plug_lo()
        cmd_list = []
        pattern_list = ['sysctl -w net.ipv4.conf.{0}.arp_ignore=0',
                        'sysctl -w net.ipv4.conf.{0}.arp_announce=0']
        for dev in ['all', consts.LOOPBACK_INTERFACE,
                    consts.NETNS_PRIMARY_INTERFACE]:
            for pattern in pattern_list:
                cmd_list.append(pattern.format(dev).split())
        for cmd in cmd_list:
            ns_exec = pyroute2.NSPopen(consts.AMPHORA_NAMESPACE, cmd,
                                       stdout=subprocess.PIPE)
            ns_exec.wait()
        ...
        self._osutils.bring_interfaces_up(ip, netcard)
        init_system = util.get_os_init_system()
        # remove multi-topology configure opts
        if init_system == consts.INIT_SYSTEMD:
            util.install_netns_systemd_service()
        return webob.Response(json=dict(
            message="OK",
            detail="{vip} has been deleted from network card {card}"
                   " successfully".format(vip=ip.exploded, card=netcard)),
            status=200)
